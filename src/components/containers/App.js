...
import { updateField, updateOption, addOption, removeOption } from '../../redux/modules/optionsForm';
...
export const App = connect(mapStateToProps)(StatefullComponent(() => {
    ...
    return ({
        ...
        return html `
            <div>
            ${PathCanvas({ linkConfig, path, width: window.innerWidth, height: window.innerHeight })}   
                ${OptionsForm({
                    speed,
                    optionsConfig,
                    options,
                    ...
                    handleSubmit: () => dispatch(updateForm({ speed, options, optionsConfig })),
                    addOption: () => dispatch(addOption()),
                    removeOption: ({ id }) => dispatch(removeOption({ id })),
                    updateOption: ({ id, fieldName, value }) => dispatch(updateOption({id, fieldName, value})),
                })}
            </div>
        `;
    };
